...
public class Camera{
    ...
    public void draw2(Program2 prog){
        prog.setUniform("projMatrix",projMatrix);
        prog.setUniform("viewMatrix",viewMatrix);
        prog.setUniform("cameraU",this.U.xyz());
        prog.setUniform("cameraV",this.V.xyz());
    ...
    public void lookAt( vec3 eye1, vec3 coi1, vec3 up1){
        ...
        W = math3d.normalize(mul(-1.0,look));
        U = math3d.normalize(cross(look,up));
        V = math3d.normalize(cross(U,look));
        compute_view_matrix();
    }
    public mat4 drawWithAdditionalMatrix(Program prog,mat4 v)
    {
        mat4 viewMatrix2=math3d.mul(v,viewMatrix);
        return viewMatrix2;
        //prog.setUniform("viewMatrix",viewMatrix2);
